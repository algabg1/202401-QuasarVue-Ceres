...
async function fetchProjects () {
  try {
    ...
    const endpoint = isAdmin.value
      ? 'http://3.90.151.136:8080/projeto/projetos'
      : `http://3.90.151.136:8080/projeto/usuario/${user.value.id}`
...
async function saveNewProject () {
  ...
  try {
    ...
    await api.post('http://3.90.151.136:8080/projeto/criar', projectData, {
      headers: { Authorization: `Bearer ${token}` }
    })
    $q.notify({
      color: 'positive',
...
async function deleteProject () {
  try {
    const token = localStorage.getItem('userToken')
    await api.delete(`http://3.90.151.136:8080/projeto/${projectToDelete.value.idProjeto}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
    $q.notify({
      color: 'positive',
