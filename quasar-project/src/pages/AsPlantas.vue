...
async function submitEdit () {
  try {
    ...
    await api.put(`{API_BASE_URL}:8080/planta/edit/${editingPlanta.value.id}`, editingPlanta.value, {
...
async function fetchPlantas () {
  try {
    const token = localStorage.getItem('userToken')
    const response = await api.get('{API_BASE_URL}:8080/planta/plantas', {
      headers: { Authorization: `Bearer ${token}` },
      params: {
...
async function excluirPlanta () {
  try {
    const token = localStorage.getItem('userToken')
    await api.delete(`{API_BASE_URL}:8080/planta/${plantaSelecionada.value.id}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
    $q.notify({
      color: 'positive',
